...
namespace MonoCross.Utilities.Encryption
{
    ...
    public class SLEncryption : BaseEncryption, IEncryption
    {
        ...
        public override void EncryptStream( Stream inputStream, Stream outputStream, string key, byte[] salt )
        {
            ...
            MXDevice.Log.Debug( string.Format( "AesEncryption.EncryptStream(stream, key, salt): Time: {0} milliseconds", DateTime.UtcNow.Subtract( dtMetric ).TotalMilliseconds ) );
        }
        ...
        public override void DecryptStream( Stream inputStream, Stream outputStream, string key, byte[] salt )
        {
            ...
            MXDevice.Log.Debug( string.Format( "AesEncryption.DecryptStream(stream, key, salt): Time: {0} milliseconds", DateTime.UtcNow.Subtract( dtMetric ).TotalMilliseconds ) );
        }
