...
namespace MonoCross.Utilities.Storage
{
    internal class BasicFileEncrypted : BasicFile, IFile
    {
        ...
        {
            if (string.IsNullOrEmpty(MXDevice.Encryption.Key))
                throw new ArgumentNullException( "Application.Encryption.Key" );
            if (MXDevice.Encryption.Salt == null)
                throw new ArgumentNullException( "Application.Encryption.Salt" );

            Rfc2898DeriveBytes rdb = new Rfc2898DeriveBytes(MXDevice.Encryption.Key, MXDevice.Encryption.Salt);
...
//                csEncrypt.Write( contents, 0, contents.Length );
            ...
            MXDevice.Log.Debug( string.Format( "BasicFileEncrypted.Save(byte[]): File: {0} Time: {1} milliseconds", filename, DateTime.UtcNow.Subtract( dtMetric ).TotalMilliseconds ) );
        ...
        public override byte[] Read( string filename )
        {
            ...
            try
            {
                ...
                MXDevice.Log.Debug( string.Format( "BasicFileEncrypted.Read: File: {0} Time: {1} milliseconds", filename, DateTime.UtcNow.Subtract( dtMetric ).TotalMilliseconds ) );
        ...
        public override string ReadString( string filename )
        {
            ...
            catch ( CryptographicException cexc )
            {
                ...
                MXDevice.Log.Error( cexc );
            }
#if !MONO
            catch ( InvalidDataException iexc )
            {
                // if we have cryptographic failure, then try the base class before giving up.
                MXDevice.Log.Error( iexc );
            }
#endif
            catch ( Exception exc )
            {
            ...
            MXDevice.Log.Debug( string.Format( "BasicFileEncrypted.ReadString: File: {0} Time: {1} milliseconds", filename, DateTime.UtcNow.Subtract( dtMetric ).TotalMilliseconds ) );
